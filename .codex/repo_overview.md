...
- **Path:** `crates/greentic-interfaces`  
  ...
  **Key updates:** adds `i18n-id` to canonical contexts, introduces the `greentic:component@0.6.0` CBOR invoke world, documents the descriptor’s `ops` metadata + optional setup contract (`docs/component-descriptor.md` + fixture), normalizes parsed dependency versions (`25.03.26` -> `25.3.26`) during staging, and keeps shared `target/wit-staging` in place to avoid concurrent build-script directory deletion races.  
